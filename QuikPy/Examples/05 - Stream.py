...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    qpProvider = QuikPy(Host='127.0.0.1')  # Вызываем конструктор QuikPy с подключением к удаленному компьютеру с QUIK

    firmId = 'SPBFUT000000'  # Фирма
    ...
    interval = 60
    print(f'{qpProvider.IsSubscribed(classCode, secCode, interval)}')
    if not qpProvider.IsSubscribed(classCode, secCode, interval)['data']:  # Если нет подписки на тикер/интервал
        qpProvider.SubscribeToCandles(classCode, secCode, interval)  # то переподписываемся
