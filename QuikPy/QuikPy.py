...
class QuikPy:
    ...
    def GetFuturesHoldings(self, trans_id=0):
        ...
        return self.process_request({'data': '', 'id': trans_id, 'cmd': 'getFuturesHolding', 't': ''})
    ...
    def process_request(self, request):
        ...
        raw_data_dict = loads(raw_data.decode('cp1251').strip())
        if raw_data_dict['cmd'] not in ('getInfoParam', 'getParamEx', 'getFuturesLimit', 'getFuturesClientHoldings', 'GetFuturesHoldings'):
            logger.debug(f'В QUIK отправлен запрос: {raw_data}')
        ...
        while True:  # Пока фрагменты есть в буфере
            ...
            if len(fragment) < self.buffer_size:  # Если в принятом фрагменте данных меньше чем размер буфера
                ...
                try:  # Бывает ситуация, когда данных приходит меньше, но это еще не конец данных
                    quik_response = loads(data)
                    if quik_response['cmd'] not in ('getInfoParam', 'getParamEx', 'getFuturesLimit',
                                                    'getFuturesClientHoldings', 'GetFuturesHoldings', 'get_candles_from_data_source'):
                        logger.debug(f'Ответ на запрос: {quik_response}')
                    return quik_response  # Попробуем вернуть ответ в формате JSON в Windows кодировке 1251
        ...
        while getattr(thread, 'process', True):  # Пока поток нужен
            ...
            for data in data_list:  # Пробегаемся по всем функциям обратного вызова
                ...
                try:  # Пробуем разобрать функцию обратного вызова
                    ...
                    # if data['cmd'] not in ('OnFirm', 'OnParam'):
                    #     logger.debug(f"{data}")
