...
class QKData(with_metaclass(MetaQKData, AbstractDataBase)):
    ...
    params = (
        ...
        ('bar23H', True),  # True - караулим последний бар сессии
    )
    ...
    def start(self):
        ...
        if self.p.sessionstart is None:  # Если время начала сессии не указано
            self.p.sessionstart = datetime.time(7, 00)  # то берем время начала сессии на бирже !!! ставлю 07 00
        if self.p.sessionend is None:  # Если время окончания сессии не указано
            self.p.sessionend = datetime.time(23, 59)  # то берем время окончания сессии на бирже
        self.put_notification(self.DELAYED, msg='Получаем исторические бары')  # Отправляем уведомление об отправке исторических (не новых) баров
        ...
        # log(f'barsCount: {barsCount}')
        if barsCount == 0:  # Если бары не получены
            self.put_notification(self.DISCONNECTED, msg='Бары не получены')  # то отправляем уведомление о невозможности отправки исторических баров
            return  # выходим, дальше не продолжаем
        self.put_notification(self.CONNECTED, msg='Успешно подключились')  # Отправляем уведомление об успешном подключении
    ...
    def _load(self):
        ...
        if self.newCandleSubscribed:  # Если получаем новые бары по подписке
            ...
            dt = datetime(jsonDateTime['year'], jsonDateTime['month'], jsonDateTime['day'], jsonDateTime['hour'],
                          jsonDateTime['min'])  # Время открытия бара
            ...
            if len(self.jsonBars) == 0:  # Если исторических данных нет (QUIK отключен от сервера брокера)
                self.put_notification(self.DISCONNECTED, msg='Исторических данных нет (QUIK отключен от сервера брокера)')  # Отправляем уведомление об окончании получения исторических баров
                return False  # Больше сюда заходить не будем
            if self.barId > self.lastBarId:  # Если получили все бары из истории
                self.put_notification(self.DISCONNECTED, msg='Получили все бары из истории')  # Отправляем уведомление об окончании получения исторических баров
                ...
                # log(f'def _load self.jsonBar: {self.jsonBar}')
            # bar_status = 'History'
        ...
        # self.lines.open[0] = o = self.store.QKToBTPrice(self.classCode, self.secCode, self.jsonBar['open'])
        # self.lines.high[0] = h = self.store.QKToBTPrice(self.classCode, self.secCode, self.jsonBar['high'])
        # self.lines.low[0] = l = self.store.QKToBTPrice(self.classCode, self.secCode, self.jsonBar['low'])
        # self.lines.close[0] = c = self.store.QKToBTPrice(self.classCode, self.secCode, self.jsonBar['close'])
        self.lines.open[0] = float(self.jsonBar['open'])
        self.lines.high[0] = float(self.jsonBar['high'])
        self.lines.low[0] = float(self.jsonBar['low'])
        self.lines.close[0] = float(self.jsonBar['close'])
        self.lines.volume[0] = float(self.jsonBar['volume'])
        self.lines.openinterest[0] = 0.0  # Открытый интерес в QUIK не учитывается
        #log_file(f'{self.p.name}, {self.liveMode}, {bar_status}, {num2date(d)}, {o}, {h}, {l}, {c}, {v}, {len(self)}')

        # Исторический бар
        # log(f'def _load bar_status: {bar_status}')
        if self.barId <= self.lastBarId:  # Если еще не получили все бары из истории
            # log(f'def _load self.barId: {self.barId}, self.lastBarId: {self.lastBarId}')
        ...
        # Чтобы не совершать сделки на истории, меняем режим торгов на историю до прихода нового бара
            ...
            if not (self.p.bar23H and str(dt)[11:13] == '23'):
                print(f'Время открытия бара {str(dt)[11:13]}')
                ...
                print(f'{datetime.now():%H:%M:%S.%f}. QKData. Сработал сценарий обработки бара в 23:30')
    ...
    def stop(self):
        ...
        if self.newCandleSubscribed:  # Если принимали новые бары и подписались на них
            self.put_notification(self.DISCONNECTED, msg='Stop')  # Отправляем уведомление об окончании получения новых баров
    ...
    def OnNewCandle(self, data):
        ...
        if jsonData['class'] != self.classCode or jsonData['sec'] != self.secCode or int(
                jsonData['interval'] != self.interval):  # Если бар по другому тикеру / временнОму интервалу
            return  # то выходим, дальше не продолжаем
        jsonDateTime = jsonData['datetime']  # Вытаскиваем составное значение даты и времени начала бара
        dt = datetime(jsonDateTime['year'], jsonDateTime['month'], jsonDateTime['day'], jsonDateTime['hour'],
                      jsonDateTime['min'])  # Переводим в формат datetime
        if date2num(dt) <= self.lines.datetime[-1]:  # Если получили предыдущий или более старый бар
            return  # то выходим, дальше не продолжаем
