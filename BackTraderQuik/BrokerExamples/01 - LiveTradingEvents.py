...
class LiveTradingEvents(bt.Strategy):
    ...
    def next(self):
        ...
        for data in self.datas:  # Пробегаемся по всем запрошенным барам
            self.log(f'{data.p.dataname} Open={data.open[0]}, High={data.high[0]}, Low={data.low[0]}, Close={data.close[0]}, Volume={data.volume[0]}')
        if self.isLive:
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    clientCode = 'SPBFUT0016z'  #'SPBFUTL9hxr'  # Код клиента (присваивается брокером)
    firmId = 'SPBFUT000000'  #'SPBFUT589000'  # Код фирмы (присваивается брокером)
    symbol = 'SPBFUT.SiZ1'
    symbols = ('SPBFUT.SiZ1', 'SPBFUT.RIZ1', 'SPBFUT.BRX1')  # Кортеж тикеров
    ...
    data = store.getdata(dataname=symbol, timeframe=bt.TimeFrame.Minutes, compression=1, fromdate=datetime(2021, 11, 25, 10, 00), LiveBars=True)  # Исторические и новые минутные бары за все время
