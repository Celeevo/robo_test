...
class LimitCancel(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:  # Если позиции нет
            ...
            self.order = self.buy(exectype=bt.Order.StopLimit, price=self.data.close[0], plimit=self.data.close[0]+2)  # Лимитная заявка на покупку
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    data = store.getdata(dataname=symbol, timeframe=bt.TimeFrame.Minutes, compression=1, fromdate=datetime(2021, 11, 29, 10, 00), LiveBars=True)  # Исторические и новые минутные бары за все время
