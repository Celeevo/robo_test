...
class QKData(with_metaclass(MetaQKData, AbstractDataBase)):
    """Данные QUIK"""
    params = (
        ('FourPriceDoji', True),  # False (по умолчанию) - не пропускать дожи 4-х цен, True - пропускать
    ...
    def _load(self):
        ...
        if not self.newCandleSubscribed:  # Если получаем исторические данные
            if len(self.jsonBars) > 0:  # Если есть исторические данные
                ...
                if not self.store.provider.IsSubscribed(self.classCode, self.secCode, self.interval)['data']:  # Если не было подписки на тикер/интервал
                    ...
                    logger.debug(f'Подписка {self.store.subscribed_symbols}')
        ...
        logger.debug(f'D-O-H-L-C-V: {num2date(self.lines.datetime[0])}, {self.lines.open[0]}, {self.lines.high[0]}, {self.lines.low[0]}, {self.lines.close[0]}, {self.lines.volume[0]}')
